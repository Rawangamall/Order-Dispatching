...
exports.assignOrder = catchAsync(async (request, response, next) => {
    ...
    const governate = await governateSchema.findOne({
    ...
    if (driver) {
        ...
        console.log("pusher triggerrrr");
        // Trigger the notification event for the specific driver
        const assignOrdersCount = await orderSchema.countDocuments({
            DriverID: driver._id,
            Status: "assign",
        });
...
const scheduleReAssignedOrder = () => {
    const updateAssignedOrders = async () => {
        try {
            ...
            const reassignedOrderIds = await orderSchema.find(
                { Status: "reassigned" },
                { _id: 1 }
            );
            console.log(reassignedOrderIds);
